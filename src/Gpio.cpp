...
Gpio::Gpio() {
	...
	std::cout << "calling wiringPiSetup() to initialize GPIO library" << std::endl;

	wiringPiSetup();

	std::cout << "Initializing GPIO internal pin representation" << std::endl;
...
Pin* Gpio::getPin(int physicalPinNumber) {
	std::cout << "Giving pin " << physicalPinNumber << std::endl;
